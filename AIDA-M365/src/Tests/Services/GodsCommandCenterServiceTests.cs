...
/// <summary>
/// Unit tests verifying workflow stage transitions, e-conomic integrations, and validation logic.
/// </summary>
public sealed class GodsCommandCenterServiceTests
{
    private readonly Mock<IGodsCalendarService> _calendar = new();
    private readonly Mock<IGodsErpService> _economic = new();
    ...
    public async Task MoveCardAsync_ShouldPatchGraph_WhenCardIsNotConfirmed()
    {
        ...
        var result = await service.MoveCardAsync(new EventCardMoveRequest(card, "planned", start, end));
    ...
    public async Task MoveCardAsync_ShouldReturnFailure_WhenEndIsBeforeStart()
    {
        ...
        var result = await service.MoveCardAsync(new EventCardMoveRequest(CreateCard(), "planned", start, start.AddMinutes(-1)));
    ...
    private CommandCenterService CreateService() =>
        new(_calendar.Object, _economic.Object, _logger.Object);

    private static GodsEventCard CreateCard() =>
        new()
        {
